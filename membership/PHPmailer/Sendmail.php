...
/*
 =================================================================================
 + 안내 : 이 파일은 아래의 웹싸이트 자료를 참고로 수정하였음
...
class Sendmail {
    ...
    function send_mail($to, $from, $subject, $body,$cc_mail=false,$bcc_mail=false) {
        ...
        //is_array($to) 함수는 $to가 배열인지 확인하는 함수
        //explode(",",$to); 문자열나누기
        //$str = "가나다라 마바사";    $str2 = explode(' ', $str);
        //결과        $str2[0]= "가나다라"  $str2[1]= "마바사"  구조로 저장되어 있음.
   
        //implode(',',$to); 함수는 문자열로 배열 원소를 결합하는 함수
        //$array = array('lastname', 'email', 'phone');
        //$rel_to = implode(",", $array);
        //echo $rel_to;   lastname,email,phone출력됨.
        if(!is_array($to)){
            $rel_to=$to;
            $to = explode(",",$to);
        }
...
$config=array(
    ...
    'smtp_id'=>'rlaeogus0911@naver.com',
    'smtp_pw'=>'star0101',
    'debug'=>1,
    'charset'=>'utf-8',
    'ctype'=>'text/plain'
